...
## Pre-release operator evidence — 9 September 2026

Before the hierarchy/overflow release, live verification found commit `7d1e066c405f1389c4d7b2647287fc8e8b126643` running as the active systemd user service `family-display.service` on the VPS Tailscale address:

```text
http://100.72.212.14:3000
```

That pre-release check confirmed HTTP 200 responses from `/healthz`, `/` and `/api/today`, and a normalized API payload containing seven days plus Previous day, no quote and one meal day. It is historical evidence of the starting state, not a claim about the currently deployed commit; always read back the live checkout after a release.

This URL is private to the tailnet. Do not create a public firewall rule, public reverse-proxy route or ordinary LAN bind for it. Repository documentation is not deployment evidence.
...
Browser verification with a crowded local fixture confirmed board and scroll dimensions of exactly 1366×768, desktop rows of 416px and 288px, all six future cards fully inside the viewport with zero scroll excess, the compact all-day dash/body/group computed to grid row 1, and a visible ellipsized long title. Computed Today/compact hierarchy values were weather text 21/16px, icon 72/32px, temperature 43.712/21px and outfit 21/16px. Screenshot capture timed out, so screenshot-based aesthetic assessment remains unverified. Physical Pi/TV overscan, clipping, cursor behaviour, viewing-distance readability, wake/reboot recovery, stale operation and midnight rollover remain outstanding.
